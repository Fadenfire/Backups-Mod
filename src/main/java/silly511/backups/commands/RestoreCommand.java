...
public class RestoreCommand extends CommandBase {
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
		...
		if (l == 1)
			...
			if (args[0].equals("blocks")) {
				if (l > 1 && l <= 7)
					return func_175771_a(args, l > 4 ? 4 : 1, pos);
				else if (l == 8)
	...
	public void restoreBlocks(ICommandSender sender, String[] args) throws CommandException {
		...
		notifyOperators(sender, this, "commands.backups.restore.area.success", changedCount, backup.time.atZone(ZoneId.systemDefault()).format(FormatHelper.dateTimeFormat));
	}
	
	public void restorePlayer(ICommandSender sender, String[] args) throws CommandException {
		...
		File playerFile = new File(new File(backup.dir, "playerdata"), player.getUniqueID() + ".dat.gz");
		...
		if (tag != null && tag.hasKey("Inventory", 9)) {
			...
			notifyOperators(sender, this, "commands.backups.restore.player.success", player.getName(), backup.time.atZone(ZoneId.systemDefault()).format(FormatHelper.dateTimeFormat));
		} else
