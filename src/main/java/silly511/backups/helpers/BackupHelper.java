...
public final class BackupHelper {
	...
	public static void trimBackups(File backupsDir) {
		...
		for (Backup backup : listAllBackups(backupsDir)) {
			...
			if (currentDay - day >= Config.trimming.perWeek) //After 8+ days, trim to every week
				shouldDelete = !set1.add((day + 3) / 7);
			else if (currentDay - day >= Config.trimming.perDay) //After 1-7 days, trim to every day
				shouldDelete = !set2.add(day);
			else if (currentSec - sec >= Config.trimming.perHour * 3600) //After 1-24 hours, trim to every hour
